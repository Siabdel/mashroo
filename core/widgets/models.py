...
class Widget(models.Model):
    ...
    title = models.CharField(max_length=100, verbose_name=_('title'))
    slug = models.SlugField(unique=True, verbose_name=_('slug'))
    region = models.ForeignKey(Region, related_name='widgets', verbose_name=_('region'), on_delete=models.DO_NOTHING)
    template = models.ForeignKey(WidgetTemplate, related_name='instances', verbose_name=_('template'), on_delete=models.DO_NOTHING)
    context = models.TextField(blank=True, null=True, validators=[validate_json], help_text=_('Use the JSON syntax.'), verbose_name=_('context'))

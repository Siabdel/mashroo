...
=> donne datetime.timedelta(4741, 66999, 829132)
ex : born = date(year=2016, month=12, day=19)
...
class MaitreDuTemps :
    ...
    def __init__(self, due_date="01/01/2020"):
        # datetime(année, mois, jour, heure, minute, seconde, microseconde, fuseau horaire)
        tab_due_date = due_date.split("/")
        self.aujourdhui = timezone.now()
        self.start_date = timezone.now()

        assert len(tab_due_date) > 2

        jour    = int(tab_due_date[0])
        mois    = int(tab_due_date[1])
        annee   = int(tab_due_date[2])
        #
        self.due_date = datetime(annee, mois, jour)

    def update_startdate(self, start_date):
        tab_start_date = start_date.split("/")
        assert len(tab_start_date) > 2
        jour    = int(tab_start_date[0])
        mois    = int(tab_start_date[1])
        annee   = int(tab_start_date[2])
        #
        self.start_date = datetime(annee, mois, jour)


    def delai_due_date(self):
        if self.due_date :
            return  (self.start_date - self.due_date ).days
        else :
            return -1


    def delai_due_date_jouvrable(self):
        if self.due_date :
            # return  (self.aujourdhui - self.due_date ).days
            jours_ouvrable = len(pd.bdate_range( self.start_date.strftime("%Y-%m-%d") , self.due_date.strftime("%Y-%m-%d") ))
    ...
    def get_jours_ouvrable(self, date1, date2):
        ...
        """
        self.aujourdhui = datetime.timezone.now()
    ...
    def nbjours_ouvrable_due_date(self):
        ...
        """
        self.aujourdhui = timezone.now()
        date1 = self.start_date.strftime("%Y-%m-%d")
    ...
    # combien de jour par mois
    def nb_jours_pmois(self):
        cal = calendar.Calendar()
        return cal.mdays

    # est-ce une année bissextile ?
    def isleap_year(self, year="2000"):
    ...
    def relativedelta(self):
        ##
        ## relativedelta(today, born)
        ## relativedelta(years=+61, months=+9, days=+28)
        rr = relativedelta(self.due_date, self.start_date)
        return rr
